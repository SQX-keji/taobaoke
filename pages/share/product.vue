				...
				<view class="index-coupon has-bg-white has-pd-10 top_30">
					<view class="goods-list" v-if="couponlist.length > 0">
						<orange-goods-card-home v-for="(item, index9) in couponlist" :logo="logo"
						 :isEnable="isEnable" :tkmoney="item.tkmoney" :tkmoneys="item.tkmoneys" :itemid="item.itemid" :itempic="item.itempic ? item.itempic + '_310x310.jpg' : 'https://tk.gomyorder.cn/logo.png'"
		...
		methods: {
			...
			loadInfos() {
				...
				this.$Request.getT('/wx/token').then(res => {
					uni.request({
						url: 'https://tk.gomyorder.cn/qrcode?access_token=' + res,
						method: 'POST',
						header: {
